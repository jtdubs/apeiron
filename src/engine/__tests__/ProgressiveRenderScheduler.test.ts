...
describe('ProgressiveRenderScheduler', () => {
  ...
  it('handles INVALIDATED state gracefully (geometry changed)', () => {
    ...
    const command1 = scheduler.update(
      initialContext,
      false,
      false,
      0,
    ...
    const command2 = scheduler.update(
      mutatedContext,
      false,
      false,
      0,
  ...
  it('handles INTERACT safe frame locking', () => {
    ...
    const commandInteract = scheduler.update(ctx, true, false, 0, 0, 0.5, 1920, 1080, 10, false);
    ...
    scheduler.update(ctx, true, false, 0, 0, 0.5, 1920, 1080, 10, false);
    expect(scheduler.getAccumulationCount()).toBe(0);
  });
  ...
  it('progresses correctly through DEEPENING and ACCUMULATING', () => {
    ...
    const cmd1 = scheduler.update(ctx, false, false, 0, 0, 1.0, 1920, 1080, 50, false); // fast ms to give high budget but not 500
    ...
    const cmd2 = scheduler.update(ctx, false, false, 0, 0, 1.0, 1920, 1080, 50, false);
    ...
    while (scheduler.getPipelineMode(false, false) === 'DEEPENING') {
      if (scheduler.getPipelineMode(false, false) === 'ACCUMULATING') break;
      scheduler.update(ctx, false, false, 0, 0, 1.0, 1920, 1080, -1, true); // send isTargetMet=true
    }

    expect(scheduler.getPipelineMode(false, false)).toBe('ACCUMULATING');

    // Now it should hit ACCUMULATING cycle 1
    expect(scheduler.getAccumulationCount()).toBe(1);

    // Pass: ACCUMULATING cycle 1, slice 1
    const cmdAccum = scheduler.update(ctx, false, false, 0, 0, 1.0, 1920, 1080, 10, true);
    ...
    const cmdAccumSubslice = scheduler.update(ctx, false, false, 0, 0, 1.0, 1920, 1080, 10, false);
