...
- **Requirement 1: Design Phase:** Explicitly document the methodology for WGSL unit testing in `docs/process/test-plan.md`. This includes defining standard unit-test data layouts (`test_in`, `test_out` buffers) and the strict `@compute fn unit_test_...` naming convention.
...
1. Update `docs/process/test-plan.md` adding "Layer 2 Flavor D: Isolated WGSL Unit Tests" describing alternate compute entry points.
...
- [x] **Documentation Sync:** Validate that `docs/process/test-plan.md` has been expanded.
