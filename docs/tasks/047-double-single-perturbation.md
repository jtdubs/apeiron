...
- [ ] Execute `WebGPUTestHarness` to verify the mathematical soundness of the new WGSL `ds_math` primitives.
- [ ] Run headless test suite to ensure standard $f32$ logic wasn't broken by the refactor.
- [ ] Render a frame at $10^{15}$ depth and compare visual pixelation/artifacting against the legacy $f32$ path.
